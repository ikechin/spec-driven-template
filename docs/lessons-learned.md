...
> **注記: 例示するパス・プロジェクト名について**
> 各セクションで `.steering/YYYYMMDD-...` 形式のディレクトリ名や具体的な実装例（Cookie domain、WHERE 句のハードコード等）が登場しますが、これらは **本テンプレートの元プロジェクト (agent-teams-sample) で実際に発生した事例** を保持したものです。採用先プロジェクトでは存在しないパスですが、学びの背景を理解するための参考としてそのまま掲載しています。採用先では新たに作成するステアリングディレクトリ（例: `.steering/20250601-feature-x/`）に置き換えて運用してください。
